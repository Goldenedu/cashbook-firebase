...
function OfficeApp() {
  const { officeEntries, setOfficeEntries, addOfficeEntry, deleteOfficeEntry, updateFirestoreDoc, bulkReplaceOffice } = useData();
  ...
  const acHeadToNames = {
    ...
    'Admin Exp': [
      'Stationary Cost',
      'Gov relate & Social Cost',
      'School Ceremony',
      'Bank Interest',
      'Donation Cost',
      'Social Exp',
      'Student Refund',
      'Building repair & Others',
      'Libray Exp',
      'Other Exp',
      'HOME: 1 Exp',
      'HOME: 2 Exp'
    ...
    'Vehicle Exp': [
      'Car Fuel & Repair',
      'Cycle Fuel & Repair',
      'Others vehicle Relate'
    ],
    'Building Exp': ['Materials', 'Labour', 'Others'],
    'Assets': ['TV, CCTV, Laptop & Others', 'Sports Materials', 'Lab & Teachig Aids'],
    'Adjustment': ['Adjustment'],
    'Drawing': ['By owner: 1', 'By owner: 2']
  };
  ...
  const generateVRNo = (dateStr) => {
    ...
    const datePrefix = `OFF-${day}${month}${year}`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editIndex !== null) {
        ...
        await addOfficeEntry(newEntry);
      }
    } catch (err) {
      console.error('Error saving office entry:', err);
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(workbook, 'Office_book.xlsx');
  };
  ...
  const importFromCSV = (e) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        const saved = await bulkReplaceOffice(mapped);
        setEntries(saved);
        alert(`Successfully imported ${saved.length} Office entries (overwritten).`);
      } catch (err) {
  ...
  // Total Advance (Advance only; depends on FY filter only)
  const totalAdvance = entries
    .filter(e => {
      const fyVal = e.fy || calculateFY(e.date || e.entryDate);
      const passFy = !filters.fy || fyVal === filters.fy;
      return passFy && (String(e.acHead || '').toLowerCase() === 'advance');
    })
    .reduce((sum, e) => sum + (parseFloat(e.amount) || 0), 0);
    ...
    <div style={{ padding: 15, backgroundColor: '#f5f6fa', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15, backgroundColor: 'white', padding: '15px 20px', borderRadius: 8, boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <h2 style={{ color: '#2c3e50', margin: 0, fontSize: 24 }}>Office Book</h2>
          <div style={{ backgroundColor: '#e74c3c', color: 'white', padding: '6px 12px', borderRadius: 6, fontWeight: 'bold', fontSize: 12 }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', margin: '6px 2px' }}>
        <h3 style={{ margin: 0, color: '#34495e', fontSize: '16px' }}>Add Office Entry</h3>
      </div>
      <form id="officeEntryForm" onSubmit={handleSubmit} style={{ backgroundColor: 'white', padding: 15, borderRadius: 8, marginBottom: 15, boxShadow: '0 4px 12px rgba(0,0,0,0.15)', border: '1px solid #e0e0e0', width: '100%' }}>
        {/* First Row - 5 fields (match widths with second row) */}
