...
function KitchenApp() {
  const { kitchenEntries, setKitchenEntries, addKitchenEntry, deleteKitchenEntry, updateFirestoreDoc, bulkReplaceKitchen } = useData();
  const entries = kitchenEntries;
  const setEntries = setKitchenEntries;

  // Dependent options: A/C Name depends on A/C Head (per request)
  const acHeadToNames = {
    'Main Cost': ['Oil', 'Rice', 'Others'],
    'Meat/Fish/Eggs/Veg': [
      'Chicken/Pork/Mutton',
      'Fish/Dried fish/Prawn',
      'Chicken',
      'Duck Eggs',
      'Vegetables',
      'Others',
  ...
  const generateVRNo = (dateStr) => {
    ...
    const datePrefix = `Kit-${day}${month}${year}`;
  ...
  const toDateObj = (dateStr) => {
    ...
    if (typeof dateStr === 'string' && /^\d{2}-\d{2}-\d{4}$/.test(dateStr)) {
      const [dd, mm, yyyy] = dateStr.split('-'); // DD-MM-YYYY
      return new Date(parseInt(yyyy,10), parseInt(mm,10)-1, parseInt(dd,10));
    }
    return new Date(dateStr); // fallback (ISO, Date)
  };

  // Sort New -> Old by date, then by VR No desc for stability
  const sortedEntries = [...filteredEntries].sort((a, b) => {
    const da = toDateObj(a.date || a.entryDate);
    const db = toDateObj(b.date || b.entryDate);
    const diff = db - da;
    if (diff !== 0) return diff;
    return String(b.vrNo || '').localeCompare(String(a.vrNo || ''));
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editIndex !== null) {
        ...
        if (existingId) {
          await updateFirestoreDoc('kitchen', String(existingId), entryToSave);
        }

        setEditIndex(null);
      } else {
        // Add flow handled by DataContext (adds id and appends to state)
        await addKitchenEntry(newEntry);
      }
    } catch (err) {
      console.error('Error saving kitchen entry:', err);
  ...
  const handleDelete = async (index) => {
    try {
      ...
      console.log('kitchen entry to delete:', entry);
      ...
      if (!entry.id) {
        ...
        await deleteKitchenEntry(entry);
        alert('Kitchen entry deleted successfully!');
      }
    } catch (error) {
      console.error('Error deleting kitchen entry:', error);
      alert(`Error deleting kitchen entry: ${error.message}. Please try again.`);
    }
  };
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Kitchen Book');
    XLSX.writeFile(workbook, 'Kitchen_book.xlsx');
  };
  ...
  const importFromCSV = (e) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        const saved = await bulkReplaceKitchen(mapped);
        setEntries(saved);
        alert(`Successfully imported ${saved.length} kitchen entries (overwritten).`);
      } catch (err) {
  ...
  const totalExp = filteredEntries.reduce((sum, entry) => sum + (parseFloat(entry.amount) || 0), 0);
  const totalBank = filteredEntries
    .filter(e => (e.method || '').toLowerCase() === 'bank')
    .reduce((sum, e) => sum + (parseFloat(e.amount) || 0), 0);
  const totalCashExp = filteredEntries
    .filter(e => (e.method || '').toLowerCase() === 'cash')
    .reduce((sum, e) => sum + (parseFloat(e.amount) || 0), 0);
  const totalKpayExp = filteredEntries
  ...
  return (
    <div style={{ padding: 15, backgroundColor: '#f5f6fa', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15, backgroundColor: 'white', padding: '15px 20px', borderRadius: 8, boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
        <h2 style={{ color: '#2c3e50', margin: 0, fontSize: 24 }}>Kitchen Book</h2>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', margin: '6px 2px' }}>
        <h3 style={{ margin: 0, color: '#34495e', fontSize: '16px' }}>Add Kitchen Entry</h3>
      </div>
      <form id="kitchenEntryForm" onSubmit={handleSubmit} style={{ backgroundColor: 'white', padding: 15, borderRadius: 8, marginBottom: 15, boxShadow: '0 4px 12px rgba(0,0,0,0.15)', border: '1px solid #e0e0e0', width: '100%' }}>
        {/* First Row - 5 fields (A/C Head read-only visible) */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 0.8fr 1fr 1fr 1fr', gap: 10, alignItems: 'center', marginBottom: 10 }}>
          <input
            type="date"
            name="date"
            value={formData.date}
          ...
          >
            {Object.keys(acHeadToNames).map(head => (
