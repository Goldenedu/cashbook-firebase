  ...
  const [filters, setFilters] = useState({
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        if (editIndex !== null) {
          ...
          // Update in Firebase
          await updateFirestoreDoc('kitchen', existingEntry.id, formData);
          ...
          await addKitchenEntry(formData);
        }
        clearForm();
        alert('Kitchen entry saved successfully!');
      } catch (error) {
        console.error('Error saving kitchen entry:', error);
        alert('Error saving kitchen entry. Please try again.');
      }
    };
    ...
    const exportToExcel = () => {
      ...
      const exportData = dataToExport.map(entry => {
        ...
        formattedEntry['A/C Head'] = entry.acHead || 'Kitchen';
        formattedEntry['A/C Class'] = entry.acClass || 'Rice';
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, 'Kitchen Expense Book');

      const dateRangeStr = dateRange.startDate || dateRange.endDate
        ? `_${dateRange.startDate || 'start'}_to_${dateRange.endDate || 'end'}`
        : '';

      XLSX.writeFile(workbook, `Kitchen_expense_book${dateRangeStr}.xlsx`);
    };
